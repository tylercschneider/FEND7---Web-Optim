...
var resizePizzas = function(size) {
  ...
  changeSliderLabel(size);
    ...
    var pizzacrazy = document.querySelectorAll(".randomPizzaContainer");
  // Iterates through pizza elements on the page and changes their widths
    for (var i = 0; i < pizzacrazy.length; i++) {
      pizzacrazy[i].style.width = newwidth + "%";
    }
  }
...
/*
The following set of functions was updating the pizzas moving in the background. The pizzas never moved in my experience even with 
the original repo. Tried multiple browsers, turning off various extensions i had running in the background as well.
Removed computationally heavy aspects and repeated look ups from large loop. Moved to outside so scope was only accessed once rather
than every time an element was being updated. Found help in the forums to get the equations right.
*/
...
function updatePositions() {
  ...
  for (var j = 0; j < items.length; j++) {
    var phase = phase_arr[j % 5];
    items[j].style.left = items[j].basicLeft + phase + 'px';
  }
